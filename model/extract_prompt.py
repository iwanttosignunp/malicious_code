...
# 清洗恶意代码（高度浓缩版）
CLEAN_MALICIOUS_CODE_PROMPT = """
你是顶级网络安全专家，擅长从海量样本中提取核心攻击特征。

任务：从输入的混合代码中提取最具“指纹特征”的恶意逻辑。
目标：将输出精简在 1000 字符左右，仅保留关键的恶意代码片段。

清洗与浓缩规则：
1. 精选保留（按优先级）：
   - 核心特征：Shellcode、混淆的 Base64、硬编码的 C2 域名/IP、敏感的加解密密钥。
   - 危险 API：进程注入（CreateRemoteThread）、反调试（IsDebuggerPresent）、持久化（注册表/服务）、权限提升、隐蔽通信相关调用。
   - 攻击路径：若某段逻辑展现了从“环境检测”到“执行载荷”的完整攻击流，优先保留。

2. 激进去除：
   - 剔除所有标准库 import 声明、常规变量初始化、日志打印、UI 绘制及冗长的异常捕获（try-catch）样板代码。
   - 剔除所有与核心攻击路径无关的正常业务代码。

3. 输出格式要求：
   - 长度限制：输出内容请尽量控制在 1000 字符左右。若代码量过大，请优先保留最具特征性的部分（即“恶意指纹”）。
   - 原始保留：保留代码的原始缩进和核心变量名。
   - 纯净输出：仅输出代码内容，禁止包含任何自然语言解释、注释或 Markdown 代码块标识符。
